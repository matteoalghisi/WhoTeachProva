    ...
    if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
        if (($iscurrentuser || is_siteadmin($USER) || !is_siteadmin($user)) && has_capability('moodle/user:update',
            ...
            /* $node = new core_user\output\myprofile\node('contact', 'editprofile', get_string('editmyprofile'), null, $url,
                $temp, null, 'editprofile');*/
				
				//s_t_mod Hui
				$button=html_writer::link($url, get_string('editmyprofile'),array('class' => 'btn btn-primary'));
				
				$node = new core_user\output\myprofile\node('contact', 'editprofile', '', null, $url,
                $button, null, 'editprofile');
			//-----------------------------------------
            ...
            if ($userauthplugin && $userauthplugin->can_edit_profile()) {
				...
				//s_t_mod Hui
				$button=html_writer::link($url, get_string('editmyprofile'),array('class' => 'btn btn-secondary'));
				
				$node = new core_user\output\myprofile\node('contact', 'editprofile', '', null, $url, $button, null, 'editprofile');
				//-----------------------------------------
            ...
            foreach ($mycourses as $mycourse) {
                if ($mycourse->category) {
                    ...
                    if (!isset($course) || $mycourse->id != $course->id) {
						...
						$params = array('id' => $mycourse->id);//s_t_mod Hui: per fare il redirect corretto al corso
                        if ($showallcourses) {
                            $params['showallcourses'] = 1;
                        }
                        //$url = new moodle_url('/user/view.php', $params);
						$url = new moodle_url('/course/view.php', $params);	//s_t_mod Hui: per fare il redirect corretto al corso
