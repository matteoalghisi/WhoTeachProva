...
foreach ($requests as $request) {
	...
	//////////
	/// PARTE PERSONALIZZATA PER ELIMINAZIONE METADATI ////_s_t_mod
	/////////
	if ($args["action"] == "delete"){
		$sql = "DELETE FROM mdl_metadata WHERE Id_resource = ".$args["id"]."";
		$DB->execute($sql);
	}
	/////PARTE DUPPLICAZIONE METADATI
	if ($args["action"] == "duplicate"){
		$sql = "INSERT INTO mdl_metadata (Id_course, Id_course_sections, Id_resource, Property, Value, grade, courseGrade, lang)
				SELECT Id_course, Id_course_sections, (SELECT MAX(id)+1 FROM mdl_course_modules ), Property, Value, grade, courseGrade, lang FROM mdl_metadata
				WHERE Id_resource = ".$args["id"]."";
		$DB->execute($sql);
	}
	/////
	//  PERSONALIZZATA FINE
