...
<html>
	...
	<body>
		...
		<center>
			...
			<h2 style="text-align:left; margin-left: 20%; margin-right:20%;"><font color="#383a3d"><?php echo convert_RS('KEYWORDS'); ?></font></h2>
			<div class = "topwords">
				<h5 style="text-align:left;  margin-left: 20%; margin-right: 20%;"><?php echo convert_RS('step3 advice'); ?> </h5>
			</div>
		...
		<div>
				...
				<center>
					<div id="risposta">
						<h4><?php echo convert_RS('Number of resources:'); ?> <b class="ka"><?php echo $first_value_pos; ?></b></h4>
				...
				<form method="post" action="<?php echo ($CFG->wwwroot . '/recommender_system/step2.php'); ?>" style="display: inline;">
					<input type="submit" name="back" id="back" value="<?php echo convert_RS('BACK'); ?>" class="btn btn-large btn-primary" style="height:40px; width:auto; padding: 5px 5px 5px 5px; font-weight:bold; border-radius: 9px; margin-right:40%;">
				</form>
				<form id="myform" name="myform" method="post" action="step4.php" style="display: inline"><input type="hidden" id="keystop" name="keywords" value=""/>
					<input type="hidden" name="token" value="<?php echo $token; ?>"/>
					<input type="submit" value="<?php echo convert_RS('CONTINUE'); ?>" class="btn btn-large" style="height:40px; width:auto; padding: 5px 5px 5px 5px; font-weight:bold; border-radius: 9px; margin-left:5%;"/>
				</form>
		...
		<script>
			$(document).ready(function () {
				...
				<?php 
					if ($keywords == null){
						echo 'alertify.confirm("<font color=\'#5392B3\' style=\'font-weight:bold; font-size:16px;\'>'.convert_RS("No keywords found").'<br />'.convert_RS("Skip this phase please or back to home").'</font>",
						function (e){
						if (e){
							$("#myform")[0].submit(); // submit form skipping jQuery bound handler
						}
						}).set("labels", {ok : "Procedi", cancel : "Home"});
						
						';
					}
				?>
				document.getElementById('alertify-ok').innerHTML = "<?php echo convert_RS('CONTINUA');?>";
				document.getElementById('alertify-cancel').onclick = function(){window.location.href = "https://www.whoteach.it/lms/recommender_system/"};
