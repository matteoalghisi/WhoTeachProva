...
class mod_recommend_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (!isset($record->numbering)) {
            $record->numbering = recommend_NUM_NUMBERS;
        }
    ...
    public function create_chapter($record = null, array $options = null) {
        ...
        if (empty($record->recommendid)) {
            throw new coding_exception('Chapter generator requires $record->recommendid');
        }
        ...
        $DB->execute($sql, array($record->recommendid, $record->pagenum));
        $record->id = $DB->insert_record('recommend_chapters', $record);

        $sql = "UPDATE {recommend}
                   SET revision = revision + 1
                 WHERE id = ?";
        $DB->execute($sql, array($record->recommendid));

        if (property_exists($record, 'tags')) {
            $cm = get_coursemodule_from_instance('recommend', $record->recommendid);
            $tags = is_array($record->tags) ? $record->tags : preg_split('/,/', $record->tags);

            core_tag_tag::set_item_tags('mod_recommend', 'recommend_chapters', $record->id,
