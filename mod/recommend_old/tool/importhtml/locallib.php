 ...
 * @param bool $verbose
 */
function toolrecommend_importhtml_import_chapters($package, $type, $recommend, $context, $verbose = true) {
    global $DB, $OUTPUT;

    $fs = get_file_storage();
    $chapterfiles = toolrecommend_importhtml_get_chapter_files($package, $type);
    $packer = get_file_packer('application/zip');
    $fs->delete_area_files($context->id, 'mod_recommend', 'importhtmltemp', 0);
    $package->extract_to_storage($packer, $context->id, 'mod_recommend', 'importhtmltemp', 0, '/');
    // $datafiles = $fs->get_area_files($context->id, 'mod_recommend', 'importhtmltemp', 0, 'id', false);
    // echo "<pre>";p(var_export($datafiles, true));

    $chapters = array();

    if ($verbose) {
        echo $OUTPUT->notification(get_string('importing', 'recommendtool_importhtml'), 'notifysuccess');
    }
    if ($type == 0) {
        $chapterfile = reset($chapterfiles);
        if ($file = $fs->get_file_by_hash(sha1("$context->id/mod_recommend/importhtmltemp/0/$chapterfile->pathname"))) {
            $htmlcontent = toolrecommend_importhtml_fix_encoding($file->get_content());
            $htmlchapters = toolrecommend_importhtml_parse_headings(toolrecommend_importhtml_parse_body($htmlcontent));
        ...
        foreach ($chapterfiles as $chapterfile) {
            if ($file = $fs->get_file_by_hash(sha1("/$context->id/mod_recommend/importhtmltemp/0/$chapterfile->pathname"))) {
                ...
                $chapter->recommendid        = $recommend->id;
                $chapter->pagenum       = $DB->get_field_sql('SELECT MAX(pagenum) FROM {recommend_chapters} WHERE recommendid = ?', array($recommend->id)) + 1;
                $chapter->importsrc     = '/'.$chapterfile->pathname;
                $chapter->content       = toolrecommend_importhtml_parse_styles($htmlcontent);
                $chapter->content       .= toolrecommend_importhtml_parse_body($htmlcontent);
                $chapter->title         = toolrecommend_importhtml_parse_title($htmlcontent, $chapterfile->pathname);
                ...
                $chapter->id = $DB->insert_record('recommend_chapters', $chapter);
                $chapter = $DB->get_record('recommend_chapters', array('id' => $chapter->id));
    ...
    if ($verbose) {
        echo $OUTPUT->notification(get_string('relinking', 'recommendtool_importhtml'), 'notifysuccess');
    }
    $allchapters = $DB->get_records('recommend_chapters', array('recommendid'=>$recommend->id), 'pagenum');
    foreach ($chapters as $chapter) {
        ...
        if (preg_match_all('/(src|codebase|name|href)\s*=\s*"([^"]+)"/i', $chapter->content, $matches)) {
            $file_record = array('contextid'=>$context->id, 'component'=>'mod_recommend', 'filearea'=>'chapter', 'itemid'=>$chapter->id);
            foreach ($matches[0] as $i => $match) {
                $filepath = dirname($chapter->importsrc).'/'.$matches[2][$i];
                $filepath = toolrecommend_importhtml_fix_path($filepath);
                ...
                if ($file = $fs->get_file_by_hash(sha1("/$context->id/mod_recommend/importhtmltemp/0$filepath"))) {
                    if (!$oldfile = $fs->get_file_by_hash(sha1("/$context->id/mod_recommend/chapter/$chapter->id$filepath"))) {
                        $fs->create_file_from_storedfile($file_record, $file);
                    }
                    $chapter->content = str_replace($match, $matches[1][$i].'="@@PLUGINFILE@@'.$filepath.'"', $chapter->content);
                }
            }
            $DB->set_field('recommend_chapters', 'content', $chapter->content, array('id'=>$chapter->id));
        }
    }
    unset($chapters);
    ...
    foreach ($allchapters as $chapter) {
        ...
        if (preg_match_all('/(href)\s*=\s*"([^"]+)"/i', $chapter->content, $matches)) {
            foreach ($matches[0] as $i => $match) {
                ...
                $chapterpath = toolrecommend_importhtml_fix_path($chapterpath);
                foreach ($allchapters as $target) {
                    if ($target->importsrc === $chapterpath) {
                        $newcontent = str_replace($match, 'href="'.new moodle_url('/mod/recommend/view.php',
        ...
        if ($newcontent !== $chapter->content) {
            $DB->set_field('recommend_chapters', 'content', $newcontent, array('id'=>$chapter->id));
        }
    }
    ...
    $recommend = $DB->get_record('recommend', array('id'=>$recommend->id));
 ...
 */
function toolrecommend_importhtml_parse_headings($html) {
}
 ...
 */
function toolrecommend_importhtml_parse_styles($html) {
    $styles = '';
 ...
 */
function toolrecommend_importhtml_fix_path($path) {
 ...
 */
function toolrecommend_importhtml_fix_encoding($html) {
    if (preg_match('/<head[^>]*>(.+)<\/head>/is', $html, $matches)) {
        $head = $matches[1];
 ...
 */
function toolrecommend_importhtml_parse_body($html) {
    $matches = null;
 ...
 */
function toolrecommend_importhtml_parse_title($html, $default) {
    $matches = null;
 ...
 */
function toolrecommend_importhtml_get_chapter_files($package, $type) {
    $packer = get_file_packer('application/zip');
