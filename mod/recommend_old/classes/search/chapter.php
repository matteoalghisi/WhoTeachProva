...
/**
 * Search area for mod_recommend chapters.
 *
...
namespace mod_recommend\search;
...
class chapter extends \core_search\base_mod {
    /**
     * @var array Cache of recommend records.
     */
    protected $recommendscache = array();
    ...
    public function get_document_recordset($modifiedfrom = 0, \context $context = null) {
        ...
        $sql = "SELECT c.*, b.id AS recommendid, b.course AS courseid
                  FROM {recommend_chapters} c
                  JOIN {recommend} b ON b.id = c.recommendid
          $contextjoin
    ...
    public function get_document($record, $options = array()) {
        try {
            $cm = $this->get_cm('recommend', $record->recommendid, $record->courseid);
    ...
    public function check_access($id) {
        ...
        try {
            $chapter = $DB->get_record('recommend_chapters', array('id' => $id), '*', MUST_EXIST);
            if (!isset($this->recommendscache[$chapter->recommendid])) {
                $this->recommendscache[$chapter->recommendid] = $DB->get_record('recommend', array('id' => $chapter->recommendid), '*', MUST_EXIST);
            }
            $recommend = $this->recommendscache[$chapter->recommendid];
            $cminfo = $this->get_cm('recommend', $chapter->recommendid, $recommend->course);
        } catch (\dml_missing_record_exception $ex) {
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/recommend/view.php', $params);
    }

    /**
     * Returns a url to the recommend.
     *
     * @param \core_search\document $doc
     * @return \moodle_url
     */
    public function get_context_url(\core_search\document $doc) {
        $contextmodule = \context::instance_by_id($doc->get('contextid'));
        return new \moodle_url('/mod/recommend/view.php', array('id' => $contextmodule->instanceid));
    }
