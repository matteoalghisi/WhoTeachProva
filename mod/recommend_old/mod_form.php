...
class mod_recommend_mod_form extends moodleform_mod {
    ...
    function definition() {
		...
		$category = $DB->get_records_sql('SELECT id FROM mdl_course_categories WHERE id = (SELECT category FROM mdl_course WHERE id = '.$PAGE->course->id.')'); 
		...
		if(is_subcategory($cat_id))
			$parent_category=get_parent_id($cat_id);
		else
			$parent_category=$cat_id;
	
		$list_category=get_all_categories($parent_category);
		...
		AND property = 'keywords' and Id_course IN(SELECT Id FROM mdl_course WHERE category IN (".implode(",",$list_category).") )"; //s_t_mod (prima era come in mod_form Backup)
...
<!-- TagCloud -->
			...
			<script type="text/javascript">	
				...
				$(document).ready(function(){
					...
					$(".mform").submit(function(){return false;});	//s_t_mod Hui: blocco l'invio del form per evitare che viene creato la risorsa 'recommender'
						
					$("#id_tagcloud").css({"height": "280px", "visibility":"hidden", "width":"80%"});
					$("#id_tagcloud").parent().jQCloud(
						word_array, 
						{
							//classPattern: 'keyboard',
							autoResize: true,
