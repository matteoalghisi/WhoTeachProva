...
if (has_capability('moodle/course:managefiles', context_system::instance()))
{
	...
	if($_SESSION['scrivi_socket'] == 1)
	{
		...
		if(isset($_POST['incomplete_courses']))
		{
			...
			echo "<script>console.log('Debug Objects: " . $row[4] . "' );</script>";
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if (isset($_POST["ka"]) || isset($_POST["incomplete_courses"]))
		{
			...
			socket_set_option($sock, SOL_SOCKET, SO_RCVTIMEO, array('sec' => 120, 'usec' => 0));
			socket_set_option($sock, SOL_SOCKET, SO_SNDTIMEO, array('sec' => 120, 'usec' => 0)); 
	...
	//la soluzione al problema cercato in questa debug stotto, sta nel fatto che il Server java non si collegava al db corretto 
	//
	//print debug
    ...
    foreach ($arr as $titolo)
    {
	   ...
	   { 
	     ...
	     if (strlen($w))
		 {
			//controllo e faccio escape di possibili caratteri speciali
			$special = array('"',"'");
			$w = str_replace($special, "", $w);
			//echo("w: ".$w);
...
<html lang="en">
	...
	<body>
	<center>
	<img src="themes/img/logo_small.png" alt="" class="responsive" style="width:300px; margin:5% 0%;"/>	<!-- s_t_mod Nuovo Stile aggiunta width e class-->
	 <!-- <br>
		<div>
			<center>
				<a href="#">
		...
		<br>
			<div id="tagcloud" class="responsive" style="height: 280px; margin:0% 15%;"></div> <!-- s_t_mod Nuovo Stile aggiunto style-->

			<center>
				<!-- <div class="controls" style="width: 25%;"> s_t_mod Nuovo Stile aggiunto margin-->
				<div class="controls" style="margin:3% 30% 0% 30%;">
					...
					?>
					<form id="formBack" name="formBack" method="post" action="<?php print $action; ?>" style="float:left;"> <!-- s_t_mod Nuovo Stile -->
						<input name="i_back" id="i_back" value="<?php print $value; ?>" style="display: none;"/>
					</form>
					<form id="formNext" name="formNext" method="post" action="step2.php" style="float:right;" /> <!-- s_t_mod Nuovo Stile -->				
				</div>
			</center>
...
?>
				...
				promise.done(function(data){
					...
					s.setAttribute('value', "<?php echo convert_RS('CONTINUE');?>");
					//s.setAttribute('class', "btn btn-large btn-primary");
					s.setAttribute('class', "btn-nuovostile-cont"); //s_t_mod Nuovo Stile
					...
					u.setAttribute('value', "<?php echo convert_RS('BACK');?>");
					//u.setAttribute('class', "btn btn-large btn-primary");
					u.setAttribute('class', "btn-nuovostile-back"); //s_t_mod Nuovo Stile
