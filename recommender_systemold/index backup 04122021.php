...
<html lang="en">
	...
	<body>
		...
		<div>
			<center>
				<a href="#">
					<!--<img src="themes/img/logo.png" alt="whoteach logo" width="40%" height="40%">-->
					<img src="themes/img/logo.png" alt="whoteach logo" width="300px"> <!-- s_t_mod Nuovo Stile -->
				</a>
			</center>
		</div>
		...
		<form method="POST" action="step1.php" name="myForm" id="myForm" margin>
			<input type="hidden" value="<?php echo $token; ?>" name="token"/>
			<table style="height:80%; width:80%; border-radius: 18px; box-shadow: 0 9px 9px rgba(0,0,0,.3);" align="center" bgcolor="#e0dddb">
				<th colspan="2">
					<h1 style = "text-align:left; margin-left:2.7%; margin-top:5%; margin-bottom:5%;">
						<font  color="#383a3d" style="font-size:30px">
							<?php echo convert_RS('COURSE FEATURES'); ?>
						</font>
					</h1>
				</th>
				...
				<tr>
					<td>
						<p><h4 style = "margin-bottom:10%; text-align:left; margin-left:5%;"><?php echo convert_RS('Course Name'); ?>:</h4></p>
					</td>
					<td>
						<center>
				...
				<tr>
					<td align="center">	
						<center>
							<h4  style = "margin-bottom:10%; text-align:left; margin-left:5%;"><?php echo convert_RS('Knowledge Area'); ?>:
						...
						<div class="controls" >
							<center>
								...
								<?php
									...
									/*$result = mysqli_query($connection, "SELECT name,id FROM mdl_course_categories" ); // modificato da Hui(01/09/2020)
									...
									while ($row = mysqli_fetch_row($result)){
										...
										if($categoryid==$row[1])
											echo '<option value="'.$row[0].'" selected=\"selected\">'.$c.'</option>';
										else
									...
									//s_t_mod Hui: per ogni sottocategoria scelta, dobbiamo ritrovare l'id e nome della macrocategoria 
									//in modo tale di utilizzare questi ultimi per la query di raccomandazione al posto della sottocategoria
									$result = mysqli_query($connection, "SELECT name,id,depth FROM mdl_course_categories" ); 
									
									while ($row = mysqli_fetch_row($result)){
									
										if($row[2]>1){
											$cat_name=get_parent_name($row[1]);
											$cat_id=get_parent_id($row[1]);
											//$c =$row[0];
											$c =get_full_path_name($row[1]);
										}
										else{
											$cat_name=$row[0];
											$cat_id=$row[1];
										...
										if($categoryid==$row[1])
											echo '<option value="'.$cat_name.'" selected=\"selected\">'.$c.'</option>';
										else
											echo '<option value="'.$cat_name.'">'.$c.'</option>';
				...
				<tr>
					<td align="center">		
						<center>
							<h4  style = "margin-bottom:10%; text-align:left; margin-left:5%;"><?php echo convert_RS('Number of Sections'); ?>:
								<span class="gestOverlayer">
									<img class="fakeLink" src="themes/img/question.png" style="width:16px;height:16px"/>
									<span class="overlayer">
										<?php echo convert_RS('Number of sections the course will contain');?>
									</span>
								</span>
							</h4>
						</center>
					...
					<!-- aggiunta in menu principale che secondario, testing
			...
			<div class="item-title"><img src="./themes/img/pignon.png" style="height:30px;"> <a class="item-link"><?php echo convert_RS('Show advanced RS options'); ?></a></div>
			...
			<center style = "margin: 8px">
				...
				<input type="reset" id="reset" value="<?php echo convert_RS('Reset Values'); ?>" class="btn btn-large btn-primary" style="float:left; margin-left:20%;"> <!-- s_t_mod Nuovo Stile volevo mettere class="btn-nuovostile-back" ma sul bottone Prosegui ho problemi-->
				...
				<input type="submit" id="continue" value="<?php echo convert_RS('Continue'); ?>" class="btn btn-large btn-primary" style="float:right; margin-right:20%;" onclick="return check();"> <!-- s_t_mod Nuovo Stile volevo mettere class="btn-nuovostile-back" ma sul bottone Prosegui ho problemi-->
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$("#myForm").submit(function(event)
				{
					...
					document.getElementById('alertify-cancel').innerHTML = "<?php echo convert_RS('Cancel');  ?>";
					document.getElementById('alertify-ok').innerHTML = "<?php echo convert_RS('CONTINUE');  ?>";
				});	
