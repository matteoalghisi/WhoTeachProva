...
//VARIABILE PER MEMORIZZARE IL NUMERO DI DISCIPLINE SELEZIONATE
...
function showSubcategory(category) {
    ...
    if (category == "Abilità informatiche") {
        ...
        $("option[name='ADHR']").css("display", "none");
        $("option[name='ALTEREGOSRL']").css("display", "none");
        $("option[name='ITSRIZZOLI']").css("display", "none");
        $("option[name='SIAM']").css("display", "none");
        $("option[name='UNIMI']").css("display", "none");
        $("option[name='UNIMIB']").css("display", "none");
        $("option[name='DEMOCLIENTI/ENTI']").css("display", "none");
        $("option[name='Noovledemo']").css("display", "none");
        $("option[name='CorsiSocialThings']").css("display", "none");
        $("option[name='SportelloAmico']").css("display", "none");
        $("option[name='INFORMATTIVASRL']").css("display", "none");
        $("option[name='SELECT']").css("display", "none");
        ...
        $("option[name='ADHR']").css("display", "");
        $("option[name='ALTEREGOSRL']").css("display", "");
        $("option[name='ITSRIZZOLI']").css("display", "");
        $("option[name='SIAM']").css("display", "");
        $("option[name='UNIMI']").css("display", "");
        $("option[name='UNIMIB']").css("display", "");
        $("option[name='DEMOCLIENTI/ENTI']").css("display", "");
        $("option[name='Noovledemo']").css("display", "");
        $("option[name='CorsiSocialThings']").css("display", "");
        $("option[name='ECDL']").css("display", "");
        $("option[name='ProvaTIMAccademy']").css("display", "");
        ...
        $("#selectSottocategoria").removeAttr("disabled");
        $("#selectSottocategoria").val("Sottocategoria");
        $("option[name='SportelloAmico']").css("display", "");

    } else if (category == "Visione imprenditoriale") {
        //PRIMA AZZERO TUTTI I VALORI PRECEDENTI
        $("option[name='ADHR']").css("display", "none");
        $("option[name='ALTEREGOSRL']").css("display", "none");
        $("option[name='ITSRIZZOLI']").css("display", "none");
        ...
        $("#selectSottocategoria").val("Sottocategoria");
        $("option[name='DEMOCLIENTI/ENTI']").css("display", "");
    } else if (category == "Sviluppo personale") {
        //PRIMA AZZERO TUTTI I VALORI PRECEDENTI
        $("option[name='ADHR']").css("display", "none");
        $("option[name='ALTEREGOSRL']").css("display", "none");
        $("option[name='ITSRIZZOLI']").css("display", "none");
        ...
        $("option[name='CorsiSocialThings']").css("display", "none");
        $("option[name='SportelloAmico']").css("display", "none");
        $("option[name='INFORMATTIVA SRL']").css("display", "none");
        $("option[name='SELECT']").css("display", "none");
        ...
        $("option[name='ADHR']").css("display", "");
        $("option[name='ALTEREGOSRL']").css("display", "");
        $("option[name='INFORMATTIVASRL']").css("display", "");
        $("option[name='SELECT']").css("display", "");
        $("option[name='SIAM']").css("display", "");
        $("option[name='DEMOCLIENTI/ENTI']").css("display", "");
    } else {
        //PRIMA AZZERO TUTTI I VALORI PRECEDENTI
...
function getRecommendations() {
    ...
    try {
        ...
        $("div[name='contenitoreCaricamento']").css("display", "none");
        $("div[name='downloadError']").css("display", "");
        return;
    }
    ...
    for (i = 0; i < recommendedResources.length; i++) {
        ...
        if (obj.title == resource) {
            ...
            if (type == "Reference Material") {
                ...
                $("#typeResourceSmartphone").append("J.Article");
            } else if (type == "Workshop and Training Material") {
            ...
            var preferenze = preferences.split(",");

            for (j = 0; j < preferenze.length; j++) {
                preferenze[j] = preferenze[j].trim();
                satisfiedPreferences.push(preferenze[j].trim().slice(1, -1));
            }

            var explanation = "Ti è stata suggerita questa risorsa perchè rispecchia le seguenti preferenze:<br>";
            $("#regolaRisorsa").append(explanation);

            for (i = 0; i < satisfiedPreferences.length; i++) {
                let preferenza = satisfiedPreferences[i];

                try {

                    if (preferenza == "disciplines") {
                        let stringaDiscipline = "Discipline --> ";
                        let discipline = JSON.parse(sessionStorage.getItem("disciplines"));

                        for (i = 0; i < discipline.length; i++) {
                            stringaDiscipline += discipline[i] + ",";
                        }

                        stringaDiscipline = stringaDiscipline.slice(0, -1);
                        $("#regolaRisorsa").append(stringaDiscipline + "<br>");
                    } else if (preferenza == "min_age") {
                        let stringaEtaMinima = "Età minima --> ";
                        let etaminima = JSON.parse(sessionStorage.getItem("minimumage"));
                        etaminima = etaminima[0];

                        if (etaminima == null) {
                            stringaEtaMinima += "1-100";
                        } else {
                            stringaEtaMinima += etaminima;
                        }

                        $("#regolaRisorsa").append(stringaEtaMinima + "<br>");
                    } else if (preferenza == "max_age") {
                        let stringaEtaMassima = "Età massima --> ";
                        let etamassima = JSON.parse(sessionStorage.getItem("maximumage"));
                        etamassima = etamassima[0];

                        if (etamassima == null) {
                            stringaEtaMassima += "1-100";
                        } else {
                            stringaEtaMassima += etamassima;
                        }

                        $("#regolaRisorsa").append(stringaEtaMassima + "<br>");

                    } else if (preferenza == "difficulty") {
                        let stringaDifficolta = "Difficoltà --> ";
                        let difficoltà = JSON.parse(sessionStorage.getItem("difficulties"));

                        for (i = 0; i < difficoltà.length; i++) {
                            stringaDifficolta += difficoltà[i] + ",";
                        }

                        stringaDifficolta = stringaDifficolta.slice(0, -1);
                        $("#regolaRisorsa").append(stringaDifficolta + "<br>");
                    } else if (preferenza == "duration") {
                        let stringaDurata = "Tempo di apprendimento --> ";
                        let durata = JSON.parse(sessionStorage.getItem("duration"));

                        for (i = 0; i < durata.length; i++) {
                            stringaDurata += durata[i] + ",";
                        }

                        stringaDurata = stringaDurata.slice(0, -1);
                        $("#regolaRisorsa").append(stringaDurata + "<br>");
                    } else if (preferenza == "language") {
                        let stringaLingue = "Lingue --> ";
                        let lingue = JSON.parse(sessionStorage.getItem("languages"));

                        for (i = 0; i < lingue.length; i++) {
                            stringaLingue += lingue[i] + ",";
                        }

                        stringaLingue = stringaLingue.slice(0, -1);
                        $("#regolaRisorsa").append(stringaLingue + "<br>");
                    } else if (preferenza == "format") {
                        let stringaFormati = "Formato risorsa --> ";
                        let formati = JSON.parse(sessionStorage.getItem("formats"));

                        for (i = 0; i < formati.length; i++) {
                            stringaFormati += formati[i] + ",";
                        }

                        stringaFormati = stringaFormati.slice(0, -1);
                        $("#regolaRisorsa").append(stringaFormati + "<br>");
                    } else if (preferenza == "type") {
                        let stringaTipi = "Tipo di risorsa --> ";
                        let tipi = JSON.parse(sessionStorage.getItem("types"));
                        ...
                        stringaTipi = stringaTipi.slice(0, -1);
                        $("#regolaRisorsa").append(stringaTipi + "<br>");
                    }
