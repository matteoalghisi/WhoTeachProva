...
function report_stats_report($course, $report, $mode, $user, $roleid, $time) {
    ...
    if ($mode == STATS_MODE_DETAILED) {
        ...
        $table->data[] =array(html_writer::label(get_string('course'), 'menucourse'), html_writer::select($courseoptions, 'course', $course->id, false));
		 ...
		 $table->data[] = array(html_writer::label(get_string('statstimeperiod'), 'menutime'), html_writer::select($timeoptions, 'time', $time, false)); 					   
		//-----------------------------------------------
	...
	echo '<input type="submit" class="btn btn-secondary" value="'.get_string('view').'" />'; //s_t_mod Hui
