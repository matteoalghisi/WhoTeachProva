    ...
    function get_content () {
        ...
        if (!isloggedin() or isguestuser()) {   // Show the block
            ...
            if (isset($CFG->rememberusername) and $CFG->rememberusername == 2) {
                ...
                $this->content->text .= '<div class="form-check" style="margin-bottom:10px;" >'; //_s_t_mod
				...
				$this->content->text .= '<input type="checkbox" name="rememberusername" id="rememberusername"
                        class="form-check-input" value="1" '.$checked.'/ style="margin-top:1px;"> '; //_s_t_mod
                $this->content->text .= get_string('rememberusername', 'admin').'</label>';
                $this->content->text .= '</div>';
            }
			
			if (!empty($forgot)) {
                $this->content->text .= '<div style="margin-bottom:20px;"><a href="'.$forgot.'">'.get_string('forgotaccount').'</a></div>';
            } // _s_t_mod A QUI** + aggiunto margin-bottom
			...
			/*_s_t_mod Bottone crea un account */

            if (!empty($signup)) {
                $this->content->text .= '<div><a class="btn btn-primary btn-clock w-100" href="'.$signup.'">'.get_string('startsignup').'</a></div>';
            }
            /*if (!empty($forgot)) {
                $this->content->text .= '<div><a href="'.$forgot.'">'.get_string('forgotaccount').'</a></div>';
            }*/ //_s_t_mod SPOSTATO DA QUI**
            ...
            if (!empty($potentialidps)) {
				...
				$this->content->text .= '<h6 style="text-align:center;">' . get_string('potentialidps', 'auth') . '</h6>';	//_s_t_mod
                $this->content->text .= '<div class="potentialidplist">';
                foreach ($potentialidps as $idp) {
                    $this->content->text .= '<div class="potentialidp">';
                    $this->content->text .= '<a class="btn btn-primary btn-block" style="padding:7px; height:auto; border-radius:0px;"'; //_s_t_mod aggiunto style
