...
$courseDest=$HTTP_STR["id_course"];

require_login();
$PAGE->set_context(context_course::instance($courseDest));
$PAGE->set_title($SITE->fullname);
$PAGE->set_heading($SITE->fullname);
$PAGE->set_url('/backup/import_mod');
...
for($i=0; $i < $numModules;$i++){
	...
	$sql="INSERT INTO mdl_course_sections(course, section, name, summary, summaryformat, visible) VALUES (?, ?, ?, ?, 1, 1)";
	$DB->execute($sql,[$courseDest, $value, $nameSection, $summary]);
	...
	if($data != "") {
			...
			for($j = 0; $j < $numeroFile; $j++){		
				...
				*/
				...
				$sql="SELECT Id_metadata, property, value FROM mdl_metadata WHERE id_course_sections = '".$id_sezione."' AND id_course IS NOT NULL AND id_resource = '".$array_resources[$j]."'";
				$fields = $DB->get_records_sql($sql);
				foreach ($fields as $field) {
					$property  = $field->property;
					$value  = $field->value;
	...
	header("Location: http://".$server_name."/lms/course/view.php?id=" .$courseDest); /* Redirect browser */
