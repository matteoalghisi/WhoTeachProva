...
{	
	...
	if(isset($_POST["keywords"]) && isset($_POST["token"]))
	{
		...
		//s_t_mod Hui: non cambio il valore della variabile $_SESSION['summary']['parts'][$partecorrente]['steps']['key']
		...
		if (!$_SESSION['summary']['parts'][$partecorrente]['steps']['key'] )
			$_SESSION['summary']['parts'][$partecorrente]['steps']['key'] =$_SESSION['keywords_selezionate'];
...
<html>
	...
	<body style="font-family:Arial, Helvetica, sans-serif;"> 
		<center>
			<img src="themes/img/logo_small.png" alt="" class="responsive" style="width:300px; margin:2% 0%;"/>	<!-- s_t_mod Nuovo Stile aggiunta width e class e modificato margin (prima era:5% 0% 2% 0%)-->
		</center>
		<!--<br>
		<div>
			<center>
		...
		<!-- s_t_mod Chiara -->
			...
			<h1 style="text-align:center; margin:0%;"> <!-- s_t_mod Nuovo Stile: -->
				<font color="#383a3d"><?php echo convert_RS('RESOURCES'); ?></font>
			</h1>
			
		<!-- s_t_mod Nuovo Stile: -->
			<div class = "topwords">
				<h5 style="text-align:center; font-size: 100%; font-weight:normal;"><?php echo convert_RS("step4 advice1");?></h5>
			</div>
			...
			<!--s_t_d rimosso da Chiara-->
			<!--<p>
					<font style="font-weight: bold;"><h2 style = "text-align:center; margin-left:10%; margin-right : 10%;"><//?php echo convert_RS('Select your Resources'); ?>
					<!-- <h3 style = "text-align:left; margin-left:10%; margin-right : 10%;"> <//?php echo convert_RS("step4 advice1");?></h3> -->
					<!--<a class="icon fa fa-question-circle text-info fa-fw " title="<//?php echo convert_RS("step4 advice1");?>"></a>
					</h2></font>
				</p>-->
			...
			<!--<form id="myformEnd" name="myformEnd" method="post" action="step5.php"> 
										...
										<td align="center">
											<div class="form-group">
												<label for="metadata_list"></label>
													<div class="controls">
														<input type="text" class="input-large" style="font-size:20px; border-radius: 6px;" id="modn" name="modn" placeholder="<?php echo convert_RS('Insert module name'); ?>" value="<?php $title = $_SESSION['course']['parts'][$partecorrente]['name']; if ($title !== 'Part not inserted yet') print $title; ?>">
													</div>
											</div>
										</td>
									</tr>
					...
					<tr>
						...
						<td align="center">
							<div class="form-group">
								<label for="metadata_list"></label>
									<div class="controls">
										<input type="text" class="input-large" style="font-size:20px; border-radius:6px; margin-left:5px;" id="modn" name="modn" placeholder="<?php echo convert_RS('Insert module name'); ?>" value="<?php $title = $_SESSION['course']['parts'][$partecorrente]['name']; if ($title !== 'Part not inserted yet') print $title; ?>">
									</div>
							</div>
						</td>	
					</tr>
				...
				<!-- s_t_mod Nuovo Stile aggiunta class -->
				<input type="button" name="back" id="back" value="<?php echo convert_RS('BACK'); ?>" class="btn-nuovostile-back">
				
				<button type="button" onclick="$('#i_back').trigger('click')" class="btn-nuovostile-home">Home</button>	<!--S_t_mod Hui-->
				
				<input type="button" value="<?php echo convert_RS('CONCLUDE'); ?>" class="btn-nuovostile-cont">
	...
	<script type="text/javascript">
		...
		///Modificato da Hui(02/09)
		//funzione per quando nn ci risorse (può essere eliminato dopo aver sistemato db)
		<?php
			if(count($id_risorsa) == 0 )
						echo "backStep();";
		?>
		
			function backStep(){
				alertify.confirm("<font color='#383a3d' style='font-weight:bold; font-size:16px;'><?php echo convert_RS('No resources found'); ?> </font>").set('labels', {ok:'BACK',cancel:"Home"});
				document.getElementById('alertify-ok').onclick = function(){window.location.href = "step3.php"};
