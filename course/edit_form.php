...
class course_edit_form extends moodleform {
     ...
     */
    function definition() {
        // global $CFG, $PAGE;		// _moodle_original_
        global $CFG, $PAGE, $DB;	// _s_t_mod_
        ...
        if (empty($course->id)) {
            if (has_capability('moodle/course:create', $categorycontext)) {
                ...
                $mform->addElement('select', 'category', get_string('coursecategory'), $displaylist,"id='course_category'"); // moodle original
				//$mform->addElement('hidden', 'category', get_string('coursecategory'), $displaylist); // _s_t_mod_
            ...
            if (has_capability('moodle/course:changecategory', $coursecontext)) {
			    ...
			    //s_t_mod Hui
				$mform->addElement('select', 'category', get_string('coursecategory'), $displaylist,"id='course_category'");
                $mform->addHelpButton('category', 'coursecategory');
            } else {
                //keep current
                $mform->addElement('hidden', 'category', null);
		...
		//s_t_mod Hui: questa parte viene usata per ricaricare la pagina quando cambia categoria scelta
		$mform->addElement('html','<input type="hidden" value=0 id="category">');
		
		// Button to update format-specific options on format change.
        $mform->registerNoSubmitButton('updatecoursecategory');
        $mform->addElement('submit', 'updatecoursecategory', 'updatecoursecategory','style="display:none"');
		...
		$("#course_category").change(function(){
			var e = document.getElementById("course_category");
			var new_category_id = e.options[e.selectedIndex].value;
			
			document.getElementById("category").value=new_category_id;

			$("#id_updatecoursecategory").click();
		...
		include "../metadata/course/edit_form.php"; //_s_t_mod_
